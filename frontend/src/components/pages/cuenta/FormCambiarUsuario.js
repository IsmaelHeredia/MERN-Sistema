...
class FormCambiarUsuario extends Component {
    ...
    handleSubmit = (e) => {
      ...
      var { _id, usuario, nuevo_usuario, clave_actual } = this.state;
      ...
      axios.post(url_acceso, {"nombre" : usuario, "clave" : clave_actual})
        .then(res => {
          ...
          if(res.data.token != null) {
            ...
            var url_cuenta = window.$url_api + "/cuenta/cambiarUsuario";

            axios.post(url_cuenta, {"id" : _id, "nombre" : nuevo_usuario}, {headers: { Authorization: `Bearer ${sessionStorage.getItem(window.$nombre_session)}` }})
            .then(res2 => {
